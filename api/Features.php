...
{	
	...
	function get_features($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT id, name, position, in_filter FROM __features AS f
									WHERE 1
									$category_id_filter $in_filter_filter ORDER BY f.position");
		$this->db->query($query);
	...
	public function get_options($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT DISTINCT feature_id, po.value FROM __options po
			$category_id_filter
			WHERE 1 $feature_id_filter $product_id_filter $brand_id_filter $features_filter ORDER BY po.value");
