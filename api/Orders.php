...
class Orders extends Simpla
{
	...
	function get_orders($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT id, delivery_id, delivery_price, separate_delivery, payment_method_id, paid, payment_date, closed, discount, date,
									...
									FROM __orders AS o 
									WHERE 1
									$status_filter $user_filter $keyword_filter $modified_from_filter ORDER BY status, id DESC $sql_limit", "%Y-%m-%d");
		$this->db->query($query);
		$orders = array();
	...
	{
		$query = $this->db->placehold("SELECT * FROM __purchases WHERE order_id=? ORDER BY id", $order_id);
		$this->db->query($query);
		$purchases = array();
