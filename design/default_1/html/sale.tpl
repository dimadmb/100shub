...
{literal}
 ...
 {/literal}
	...
	<li class="product">
		...
		{if $product->image}
		<div class="image">
                 <a href="http://100shub.ru/products/{$product->url}"><img src="{$product->image->filename|resize:200}" alt="{$product->name|escape}"/></a>
		</div>
		{/if}
		<!-- Фото товара (The End) -->

		<!-- Название товара -->
		<h3><a product_id="{$product->id}" href="http://100shub.ru/products/{$product->url}">{$product->name|escape}</a></h3>
