...
<div id="brands">
	{foreach name=brands item=b from=$category->brands}
		...
		{/if}
		<a brand_id="{$b->id}" href="http://100shub.ru/catalog/{$category->url}/{$b->url}{$filter_params}" {if $b->id == $brand->id}class="selected"{/if}>{$b->name|escape}</a>
	{/foreach}
</div>
{/if}
...
{literal}
 ...
 {/literal}
	...
	<li class="product">
		...
		{if $product->image}
		<div class="image">
                <a href="http://100shub.ru/products/{$product->url}" target="_self" ><img src="{$product->image->filename|resize:200}" alt="{$product->name|escape}"/></a>
		</div>
		{/if}
		<!-- Фото товара (The End) -->

		<!-- Название товара -->
		<h3><a product_id="{$product->id}" href="http://100shub.ru/products/{$product->url}" target="_self">{$product->name|escape}</a></h3>
		...
		<form class="cart" method="get" action="cart">
			...
			<!-- В корзину -->
            <a class="add_to_cart" style="text-align:center; line-height:20px;" href="http://100shub.ru/products/{$product->url}">Подробнее</a>
