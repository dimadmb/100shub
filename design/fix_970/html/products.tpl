...
<div id="features">
	...
	<div class="feature" >
		<b style="line-height:24px;" feature_id="{$f->id}">{$f->name}:</b>
		<form >
			<select  name="{$f->id}" id="">
				<option value="{url params=[$f->id=>null, page=>null]}" {if !$smarty.get.$f@key} selected {/if}>Все</option>
				{foreach $f->options as $o}
				<option value="{url params=[$f->id=>$o->value, page=>null]}" {if $smarty.get.$f@key == $o->value} selected {/if} >{$o->value}</option>
				{/foreach}
			</select>
		</form>
	</div>
