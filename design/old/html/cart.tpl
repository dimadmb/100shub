...
<form method=post name=cart>
    <table id="cart_products">
        ...
        <tr>
            <td class="td_1">
              <a href="products/{$purchase->product->url}">{$purchase->product->name|escape}</a> {$purchase->variant->name}
            </td>
            <td class="td_2">{$purchase->variant->price|convert}&nbsp;{$currency->sign}</td>
            <td class="td_3">
                <p><select name=amounts[{$purchase->variant->id}] onchange="document.cart.submit_order.value='0'; document.cart.submit();">
                ...
                <a href='cart/remove/{$purchase->variant->id}' title='убрать из корзины'><img src='design/{$settings->theme}/images/delete.png' alt='убрать из корзины' align="absmiddle"></a>
                </p>
            </td>
            <td class="td_4">
    ...
    <div class="total_line">
        ...
        {/if}
        {if $user->discount}
        <span class=total_sum>Скидка: <span id=subtotal_price>{$user->discount}</span>&nbsp;%
        {/if}
        <span class=total_sum>Итого: <span id=subtotal_price>{$cart->total_price|convert}</span>&nbsp;{$currency->sign}
    </div>
        ...
        <table class=order_form>
            ...
            <tr>
                ...
                <td>
                    ...
                    <input type="submit" name="checkout" value="Заказать" id="order_button"/>
                    </p>
