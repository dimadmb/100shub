	...
	public function fetch()
	{
		if($this->request->method('post'))
		{
			...
			if($order->id)
			{
				...
				$delete_ids = array_diff(array_keys($this->orders->get_purchases($order->id)), $purchases_ids);
				foreach($delete_ids as $delete_id)
					$this->orders->delete_purchase($delete_id);
				...
				if($new_status == 0)					
				{
					if(!$this->orders->open(intval($order->id)))
						$this->design->assign('message_error', 'error open');
					else
				...
				elseif($new_status == 1)					
				{
					if(!$this->orders->close(intval($order->id)))
						$this->design->assign('message_error', 'error closing');
					else
		...
		$subtotal = 0;
		$purchases_count = 0;
		if($order && $purchases = $this->orders->get_purchases($order->id))
		{
			// Покупки
			$products_ids = array();
			$variants_ids = array();
