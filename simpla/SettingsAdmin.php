...
{	
	...
	{	
		...
		if(isset($_POST))
		{
			...
			$this->settings->prozent_stavka = $this->request->post('prozent_stavka');
			$this->settings->news_text = $this->request->post('news_text');
			...
			if(!empty($watermark) && in_array(pathinfo($watermark, PATHINFO_EXTENSION), $this->allowed_image_extentions))
			{
