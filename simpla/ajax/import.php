...
{	
	...
	public function import()
	{
		...
		$columns = fgetcsv($f, 1024, $this->column_delimiter);	
		...
		{ 
			// Читаем строку
			$line = fgetcsv($f, null, $this->column_delimiter);
	...
	private function import_item($item)
	{
		...
		if(!empty($category_id))
 		...
 		foreach($item as $feature_name=>$feature_value)
 		{
 			...
 			if(!$this->internal_column_name($feature_name))
 			{ 
