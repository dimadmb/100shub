...
<div id="right_menu">
	...
	<ul>
		...
		<li {if $filter=='hits'}class="selected"{/if}><a href="{url keyword=null brand_id=null category_id=null page=null filter='hits'}">Лучшие товары</a></li>
		<li {if $filter=='discounted'}class="selected"{/if}><a href="{url keyword=null brand_id=null category_id=null page=null filter='discounted'}">Товары со скидкой</a></li>
	</ul>
	<!-- Фильтры -->
...
$(function() {
	...
	$("a.hit").click(function() {
		...
		$.ajax({
			type: 'POST',
			url: 'ajax/update_object.php',
			data: {'object': 'product', 'id': id, 'values': {'hit': state}, 'session_id': '{/literal}{$smarty.session.id}{literal}'},
			success: function(data){
				icon.removeClass('loading_icon');
				if(state)
					line.addClass('hit');				
