...
class FeedbackView extends View
{
	function fetch()
	{
        ...
        elseif($this->request->method('post') && $this->request->post('callback')){
            $feedback->callback_name         = $this->request->post('name_callback');
			$feedback->callback_phone        = $this->request->post('phone_callback');
            
            $this->design->assign('callback_name',  $feedback->callback_name);
			$this->design->assign('callback_phone', $feedback->callback_phone);
            
            if(empty($feedback->callback_name))
				$this->design->assign('error_callback', 'empty_callback_name');
			elseif(empty($feedback->callback_phone))
			{
				$this->design->assign('error_callback', 'empty_callback_phone');
			}
			else
			{
				$this->design->assign('callback_sent', true);
				
                $message = "Имя: ".$feedback->callback_name."\n"."Телефон: ".$feedback->callback_phone;
                
                $headers = "Content-type: text/html; charset=utf-8\r\n";

                
				mail("12nas24@mail.ru", "Callback - 100shub.ru", $message,$headers);
