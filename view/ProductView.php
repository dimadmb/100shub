...
class ProductView extends View
{
	...
	{   
		...
		$variants = $this->variants->get_variants(array('product_id'=>$product->id, 'instock'=>true));		
		// Скидка
		$discount = 0;
		...
		foreach($this->products->get_related_products($product->id) as $p)
			$related[$p->related_id] = $p;

		if(!empty($related))
		{
			$related_products = array();
			foreach($this->products->get_products(array('id'=>array_keys($related), 'in_stock'=>1, 'visible'=>1)) as $p)
		...
		$this->design->assign('category', reset($products->categories));		
